/*
    ...
    사원정보는 사원테이블 , 직급정보는 직급테이블 , 등등 -> 중복을 최소하시키기위해.
    => 즉 , JOIN 구문을 이용해서 여러개 테이블 간의 "관계"를 맺어서 같이 조회해아함.
    => 단 , 무작정 JOIN을 하는것이 아니라 테이블 간에 "연결고리"에 해당되는 칼럼을 매칭시켜서 조회해아함.
...
--> 조인을 통해서 연결고리에 해당되는 칼럼들만 제대로 매칭시키면 하나의 RESULTSET으로 조회가능해진다.
...
-- UNSING구문 : 컬렴명이 동일한 경우에만 사용 가능 , 동일한 컬럼명 하나만써주면 알아서 매칭시켜줌(AMBIGUIOUSLY 발생X)
...
-- 1. 부서가 인사관리분인 사원들의 사번 , 사원명 , 보너스를 조회.
--> 오라클전용구문
SELECT
    emp_id,
    emp_name,
...
/*
    ...
    단 , 반드시 LEFT / RIGHT를 지정해야한다. => 기준이 되는 테이블을 지정해준다.
    
    일치하는행 + 기준이되는 테이블을 기준으로 일치하지 않는 행도 포함시켜서 조회 시켜줌.
*/
...
-- EMPLOYEE테이블을 기준으로 조회했기 때문에 , EMPLOYEE에 존재하는 데이터는 일치하든 일치하지 않든 다 조회하게끔한다.
...
FROM EMPLOYEE , DEPARTMENT
WHERE DEPT_CODE = DEPT_ID(+);
-- 내가 기준으로 삼을 테이블의 컬렴명이 아닌 , 반대 테이블의 컬럼명에 (+)을 붙여준다.

-- 2) RIGHT OUTER JOIN : 두 테이블의 오른편에 기술된 테이블을 기준테이블 삼겠다.
...
-- 일치하는 행들 + LEFT OUTER JOIN 기준 새롭게 추가된행 + RIGHT OUTER JOIN 기준 새롭게 추가된 행들
...
-- USING구문은 등가조인(동등비교 '=')에서만 컬럼명이 동일한 경우만 사용가능하다. -> 비등가 조인은 ON 구문만 사용한다.
...
/*
    ...
    => 자체 조인의경우 테이블에 반드시 별칭을 붙여줘야한다.(서로 다른 테이블인것처럼 조인시키기 위함)
*/
SELECT * FROM EMPLOTEE E; -- 사원에대한 정보를 도출용 테이블
SELECT * FROM EMPLOYEE M; -- 사수에대한 정보를 도출용 테이블
