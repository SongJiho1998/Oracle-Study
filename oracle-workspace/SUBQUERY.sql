/*
    ...
    메인 SQL문을 위해서 '보조'역활을 하는 SELECT문.
...
/*
    서브쿼리 구분
    서브쿼리를 수행한 결과값이 몇행 몇일 이냐에 따라서 분류됨.
...
WHERE SALARY < (SELECT
                FLOOR(AVG(SALARY))
                FROM EMPLOYEE);
-- 2. 최저급여를 받는 사원의 사번 , 사원명 , 직급코드 , 급여 , 입사일 조회
SELECT
    emp_id,
    emp_name,
    job_code,
...
-- 노옹철사원의 급여보다 더 많이받는 사원들의 사번 , 이름 , 부서명 , 급여 조회
...
-- 부서별 급여 합이 가장 큰 부서 하나만을 조회, 부서코드, 부서명 , 급여의합.

-- 1) 각 부서별 급여 합 구하기 + 가장큰 합을 찾기.
SELECT 
    dept_code,
    SUM(salary)
FROM EMPLOYEE
...
/*
    ...
    - < ANY (10 , 20 , 30) 서브쿼리 : 여러개의 결과값 중에서 "하나라도" 작을 경우
                                     즉 , 여러개의 결과값 중에서 가장 큰 값보다 작 을경우
                                     
    - > ALL (10 , 20 , 30) 서브쿼리 : 여러개의 결과값의 "모든" 값보다 클 경우
                                     즉 , 여러개의 결과값중에서 가장 큰값보다 클 경우
    - < ALL (10 , 20 , 30) 서브쿼리 : 여러개의 결과값이 "모든" 값보다 작을 경우
...
-- 1. 선동일 또는 유재석 사원과 같은 부서인 사원들을 조회하시오 (사원명 , 부서코드 , 급여)
SELECT
    emp_name,
    dept_code,
    salary
...
-- 1) 과장 직급의 급여들 조회 -> 다중행 , 단일행
SELECT
    salary
FROM EMPLOYEE E
JOIN JOB J ON (E.JOB_CODE = J.JOB_CODE)
WHERE J.JOB_NAME = '과장';
-- 2) 위 급여들보다 "하나라도" 높은급여 있다면 그 직원들 조회(대리직급만).
SELECT 
    emp_id,
    emp_name,
    job_name,
...
/*
    3. (단일행) 다중열 서브쿼리.
    서브쿼리 조회 결과가 값은 한행 이지만 , 나열된 칼럼의 갯수가 여러개일 경우.
*/
...
-- 4) 단일행 다중열 서브쿼리로 바꾸기.
-- [표현법] (비교할 대상 컬럼1 , 비교할 대상 컬럼2) = (비교할 값 1 , 비교할 값 2 => 서브쿼리 형식으로 제시)
-- 비교한 값의 순서를 맞춰줘야함.
SELECT 
    emp_name,
    dept_code,
...
SELECT
    ...
    salary AS "급여"
FROM EMPLOYEE
WHERE (NVL(DEPT_CODE , '없음') , SALARY) IN (SELECT
                                    NVL(DEPT_code , '없음') AS "부서코드",
                                    MAX(salary) AS "급여"
                                FROM EMPLOYEE
                                GROUP BY DEPT_CODE);
 ...
 /*
    ...
    서브쿼리를 수행한 결과(RESULTEST)을 테이블 대신 사용하는방법.
 */   
...
SELECT
    emp_id,
    emp_name,
    ((salary + (salary * NVL(bonus , 0))) * 12) AS "보너스포함 연봉", 
    dept_code
FROM EMPLOYEE
WHERE ((salary + (salary * NVL(bonus , 0))) * 12) >= 30000000;

-- 인라인뷰를 사용 : 사원명만 골라내기(보너스포함 연봉이 3000만원 이상인 사원들중에)
SELECT
    emp_name
FROM (SELECT
         emp_id,
         ...
         job_code
      FROM EMPLOYEE
      WHERE ((salary + (salary * NVL(bonus , 0))) * 12) >= 30000000) B
WHERE B.JOB_CODE = 'J1';

-- 인라인뷰를 주로 사용하는 예
-- TOP-N 분석 : 데이터베이스 상에 있는 자료중 최 상위 N개의 자료를 보기위해 사용하는 기능.
...
-- 해결 방법 : ORDER BY로 정렬한 테이블을 가지고 ROWNUM순번을 부여 후 5등까지만 추림.
SELECT
    ROWNUM,
    emp_name,
    salary
...
FROM (
        SELECT
             dept_code AS "부서코드",
             TRIM(TO_CHAR(FLOOR(AVG(SALARY)) , 'L999,999,999')) || '원' AS "평균"
        FROM EMPLOYEE
        GROUP BY DEPT_CODE
        ORDER BY FLOOR(AVG(SALARY)) DESC
      ) 
...
-- 입사일 기준 미래 ~ 과(내림차순) , 순번 부여후 5명
SELECT
    ROWNUM,
    E.* -- 인라인뷰에서 *을 사용하려면 반드시 별칭이 부여되어 있어야함.
FROM 
        (
            SELECT
               emp_name,
...
/*
    ...
    RANK() OVER(정렬 기준) : 공동 1위가 3이라고하면 그 다음순위는 4위로 하겠다.
    DENSE_RANK() OVER(정렬 기준) : 공동 1위가 3이라고하면 그 다음순위는 2위로 하겠다. 
...
SELECT
    ...
    DENSE_RANK() OVER(ORDER BY SALARY DESC) AS "순위"
FROM EMPLOYEE
WHERE DENSE_RANK() OVER(ORDER BY SALARY DESC) <= 5; -- 오류남
