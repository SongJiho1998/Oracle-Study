...
-- 계정 생성
-- 일반 사용자계정을 만들 수 있는 권한은 오로지 관리자 계정에만 있음.
-- 사용자 계정 생성방법.
...
-- 사용자 계정 : DB에 대해서 질의 갱신, 보고서 작성등의 작업을 수행할 수 있는 계정 , 업무에 필요한
--             최소한의 권한만 가지는것을 원칙으로함

/*
    ROLE : 권한
    CONNECT : 사용자가 데이터베이스에 접속 가능하도록 하기 위한 CREATE SESSIONN 권한이 있는 ROLE.
    RESOURCE : CREATE구문을 통해 겍체를 생성 할 수 있는 권한과, INSERT, UPDATE, DELETE구문을 사용할수
    있는 권한을 모아놓은 ROLE.
