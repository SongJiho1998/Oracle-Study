/*
    ...
    EX) 회원 탈퇴시 기존의 회원테이블에 데이터 DELETE후 곧 바로 탈퇴된 회원들만 따로 보관하는 테이블에 자동으로
    INSERT처리.
    ...
    * 트리거의 종류
    SQL문의 시행시기에 따른 분류
     > BEFORE TRIGGE : 내가 지정한 테이블에 이벤트가 발생되기전에 트리거 실행.
     > AFTER TRIGGER : 내가 지정한 테이블에 이벤트가 발생된 후에 트리거 실행.       
     
     SQL문에 의해 영향을 받는 각 행에 따른 분류
      > STATEMENT TRIGGER(문장트리거) : 이벤트가 발생한 SQL문에 대한 딱 한번만 트리거 실행
      > ROW TRIGGER(행 트리거) : 해당 SQL문 실행할때마다 매번 트리거 실행(FOR EACH ROW옵션 기술해야함)
         사용가능 옵션
         > : OLD - BEFORE UPDATE , DELETE (수정전 자료)
         > : NEW - AFTER INSERT , UPDATE (추가된 자료)
...
-- EMPLOYEE테이블에 새로운 행이 INSERT될때마다 자동으로 메세지 출력되는 트리거를 정의
SET SERVEROUTPUT ON;
...
VALUES(SEQ_EID.NEXTVAL , '송지효' , '111111-1111111' , 'D1' , 'S1' , 'J1' , SYSDATE);                                                                                                   
...
-- 테이블생성 (상품에 대한 데이터를 보관한테이블) TB_PRODUCT
...
-- 상품번호 중복 안되게끔 매번 새로운번호를 발생시키는 시퀀스 생성 SEQ_PCODE
...
--    어떤상품이 어떤날자에 몇개가 입고 또는 출고가 되었는지 기록하는 용도.
...
-- TB_RPODUCT테이블에 매번 자동으로 재고수량 UPDATE 되게끔 트리거를 정의

/*
    - 상품이 입고된 경우 -> 해당하는 상품을 찾아서 재고수량 증가 UPDATE
    UPDATE TB_PRODUCT
        SET STOCK = STOCK + 추가된 수량(TB_PRODETAIL테이블에 INSERT시 사용한 값.)
    WHERE PCODE = 입고한 상품번호(TB_PRODETAIL테이블에 INSERT시 사용한 값.)
    
    - 상품이 출고된 경우 -> 해당상품 찾아가서 재고수량 감소 UPDATE
    UPDATE TB_PRODUCT
        SET STOCK = STOCK - 출고된수량(TB_PRODETAIL에 INSERT할때 사용한 값.)
...
/*
    ...
    트리거 단점
    1. 번번한 추가 , 수정 , 삭제시 ROW의 삽입 , 추가 , 삭제가 함께 실행되므로
    성능상 좋지않음.
    2. 관리적 측면에서 형상관리가 불가능하기때문에 관리하기가 불편한다.
    3. 트리거를 남용하게 되는경우 예상치 못한 사태가 발생할수있으며 유지보수가 힘
    들다.
*/
