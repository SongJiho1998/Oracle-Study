/*
    ...
    <테이블수정>
    [표현법]
    ALATER TABLE 테이블명 수정할내용;
    
    -- 수정할내용
    1) 칼럼추가 / 수정 / 삭제
    2) 제약조건 추가 / 삭제 => 수정은 불가(수정하고자한다면 삭제 후 새로이 추가)
    3) 테이블명 / 칼럼명 / 제약조건명 수정
*/

-- 1) 칼럼 추가 / 수정 / 삭제
-- 1-1) 칼럼 추가 (ADD) : ADD 추가할 칼럼명 자료형 DEFAULT 기본값. (단 , DEFAULT기본값은 생략가능)                                                                             

SELECT * FROM DEPT_COPY;

-- CNAME 칼럼 추가.
ALTER TABLE DEPT_COPY ADD CNAME VARCHAR2(20);
-- 새로운 칼럼이 만들어지고 기본적으로 NULL값으로 채워짐.

-- LNAME 칼럼추가 DEFAULT 지정.
ALTER TABLE DEPT_COPY ADD LNAME VARCHAR2(20) DEFAULT '한국';
-- 새로운 칼럼 추가하고 기본값으로 한국으로 채워짐.

-- 1-2) 칼럼 수정(MODIFY)
--      칼럼의 자료형 수정 : MODIFY 수정할컬럼명 바꾸고자하는 자료형
--      DEFAULT 값 수정 : MODIFY 수정할컬럼명 DEFAULT 바꾸고자하는 기본값.

-- DEPT_COPY테이블의 DEPT_ID칼럼의 자료형을 CHAR(3)으로 변경.
ALTER TABLE DEPT_COPY MODIFY DEPT_ID CHAR(3);

-- 현재 변경하고자 하는 칼럼에 이미 담겨있는 값과 완전히 다른 타입으로는 변경이 불가능하다.
-- DEPT_COPY테이블의 DEPT_ID칼럼의 자료형을 NUMBER 변경.
ALTER TABLE DEPT_COPY MODIFY DEPT_ID NUMBER;
-- column to be modified must be empty to change datatype
-- 현재 병경하고자하는 컬럼에 이미 담겨있는 값과 일치하는 칼럼이거나 혹은 속해있는칼럼 , 그리고 더 큰 바이트의 자료형으로만
-- 변경가능함.
...
-- 한번에 여러개 칼럼 자료형 변경하기
-- DEPT_TITLE의 데이터타입을 VARCAHR2(40)으로
-- LOCATION_ID의 데이터타입을 VARCARH2(2)로 
-- LNAME칼럼의 기본값을 '미국'
...
ROLLBACK;
-- 1-3) 칼럼 삭제 (DROP COLUMN) : DROP COLUMN 삭제하고자하는 칼럼명
...
-- 테이블에 최소 한개의 칼럼은 존재해야함.
/*
    2)제약조건 추가 / 삭제
    2-2) 제약조건 추가
    
    - PRIMARY KEY : ADD PRIMARY KEY(컬럼명);
    - FOREIGN KEY : ADD FOREIGN KEY(컬럼명) PEFERENCES 참조할테이블명[(참조할 컬럼명)]
    
    - UNIQUE : ADD UNIQUE(컬럼명)
    - CHECK : ADD CHECK(컬럼에 대한 조건);
    - NOT NULL : MDOIFY 컬럼명 NOT NULL;
...
-- DEPT_COPY테이블로부터 PK에 해당하는 제약조건을 삭제.
ALTER TABLE DEPT_COPY DROP CONSTRAINT SYS_C007197;

-- DEPT_COPY테이블로부터 UNIQUE에 해당하는 제약조건을 삭제
...
-- 부모테이블 삭제하는 경우에 대한 테스트.
-- 1. DEPT_TEST에 DEPT_ID를 PRIMARY KEY 제약조건 추가.
ALTER TABLE DEPT_TEST
ADD CONSTRAINT DCOPY_PK PRIMARY KEY(DEPT_ID);
-- 2. EMPLOYEE_COPY3에 외래키 (DEPT_CODE)를 추가 (외래키 이름 : ECOPY_FX)
