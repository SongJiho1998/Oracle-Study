/*
     ...
     * 트랜잭션 종류
     - COMMIT; (실행시) : 하나의 트랜잭션에 담겨있는 변경사항들을 실제 DB에 반영하겠다는것을 의미.        
                       실제 DB에 반영시킨 후 트랜잭션은 비워짐 -> 확정
     - ROLLBACK; (실행시) : 하나의 트랜잭션에 담겨있는 변경사항들을 실제 DB에 반영하지 않겠다는것을 의미.                                         
                           트랜잭션에 담겨있는 변경사항도 다 삭제한 후 마지막 COMMIT시점으로 돌아감.                                     
     - SAVEPOINT 포인트명; (실행시) : 현재 이 시점에 임시저장점을 정의해둠.
     - ROLLBACK TO 포인트명; (실행시) : 전체 변경사항들을 삭제(마지막COMMIT시점까지 돌려놓음) 하는것이아니라
...
ROLLBACK; -- 트랜잭션의 대상이 되는 SQL문중에 남아있는게 없어서 롤백시켜도 차이가 없음.       
--------------------------------------------------------------------------------

-- 1. EMP_01테이블에서 사번이 217 , 216 , 214인 사원만 삭제
DELETE FROM EMP_01
WHERE EMP_ID IN (214 , 216 , 217);
-- 2. 3개행이 삭제된 시점에 SAVEPOINT 지정.
SAVEPOINT SP1;
-- 3. EMP_01테이블에 사번이 218번인 사원 삭제.
DELETE FROM EMP_01
WHERE EMP_ID = 218;

-- 4. EMP_01테이블에 사번 801 , 이름 송지호 , 부서 인사부인 사원을 추가
INSERT INTO EMP_01 VALUES(801 , '송지호' , '인사부');
...
SELECT * FROM EMP_01; -- 22명.
-- 마지막 COMMIT시점 기준으로 마지막으로 SELECT한 시점과 동일한결과가 나옴.
...
/*
    ...
    => 즉 , DDL수행전 변경사항이 있다면 정확히 픽스(COMMIT OR ROLLBACK)을 하고 DDL을 실행해야한다. 
*/
--------------------------------------------------------------------------------
